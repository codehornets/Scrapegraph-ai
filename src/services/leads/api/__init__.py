...
class Api:
    def __init__(
        ...
        self._create_response_files = (
            create_response_files  # Flag for creating response files
        )
        if not self.is_api_running():
            raise ApiException(
                f"API at {self._api_url} is not running. Please check if the API is up and running."
            )
...
api = Api('https://example.com')""".format(
                    self._api_url
                )
            )
    ...
    def create_async_task(self, data, scraper_name=None):
        ...
        self._write_json(
            "create_async_task", response_data
        )  # Call write_json with method name
    ...
    def create_sync_task(self, data, scraper_name=None):
        ...
        response_data = response.json()
        self._write_json(
            "create_sync_task", response_data
        )  # Call write_json with method name
        return response_data
    ...
    def create_async_tasks(self, data_items, scraper_name=None):
        ...
        response_data = response.json()
        self._write_json(
            "create_async_tasks", response_data
        )  # Call write_json with method name
        return response_data
    ...
    def create_sync_tasks(self, data_items, scraper_name=None):
        ...
        response_data = response.json()
        self._write_json(
            "create_sync_tasks", response_data
        )  # Call write_json with method name
        return response_data
    ...
    def get_task_results(
        self, task_id, filters=None, sort=None, view=None, page=1, per_page=None
    ):
        """
