...
from src.models.lead import LeadsResult, SearchQuery
from src.services.caches import CacheService
from src.services.leads.api import Api, ApiException
from src.services.leads.segments import SegmentService
...
class LeadService:
    ...
    def generate_leads(
        ...
        zoom_level: int = 14,
        max_keywords: int = 3,  # TODO: Optional limit on the number of keywords processed remove
        ...
        for keyword, intent, score in keywords_to_process:
            cache_key = self.cache_service.generate_cache_key(
                f"{keyword}_{intent}_{self.industry}_{self.base_city}"
            )
            ...
            for query in validated_queries:
                ...
                task_results = queries["task_results"]
                leads = task_results["results"]

                try:
                    all_leads.append(leads)
                    found_leads.extend(leads)

                    segmented_leads = self.segment_service.segment_leads_in_batches(
                        leads
                    )
                    ...
                    store_leads(
                        ...
                        segmented_leads=segmented_leads,
                    )

                    lead_tracking.append(
                        {
                            "query": query,
                            "keyword": keyword,
                            "intent": intent,
    ...
    def execute_task(self, query: str, data, scraper_name="google_maps_scraper"):
        try:
            ...
            task_results = self.api.get_task_results(task["id"])
    ...
    def parse_leads_data(self, data: Dict[str, Any]) -> Optional[LeadsResult]:
        try:
            leads_data = LeadsResult(**data)
            return leads_data
...
#     marketing_goals=["awareness", "consideration", "conversion"],
#     marketing_strategies=["general lead generation", "lead nurturing", "cold email outreach"],
#     locations=["us", "ca"],
#     industry="consulting",
