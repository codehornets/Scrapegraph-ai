...
FRENCH_STOPWORDS = stopwordsiso.stopwords(["fr"])
...
def normalize_fr_text(text, rules=None):
    if not text:
        return text
    ...
    transformations = OrderedDict(
        [
            ...
            (
                "remove_french_stopwords",
                lambda text: " ".join(
                    word for word in text.split() if word not in FRENCH_STOPWORDS
                ),
            ),
