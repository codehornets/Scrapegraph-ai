...
class ChromeVersions(VersionScraper):
    resource: str = (
        "https://raw.githubusercontent.com/vikyd/chromium-history-version-position/master/json/all-version.json"
    )
...
class FirefoxVersions(VersionScraper):
    ...
    def download(self) -> List[str]:
        ...
        versions = [
            ver
            for ver in versions
            if self.leading_num(ver) >= highest_ver - self.threshold
        ]
        self.write_file(versions)
        return versions
