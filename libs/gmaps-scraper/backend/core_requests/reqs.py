        ...
        **kwargs,
        ...
        if kwargs:
            ...
            if session and sess_kwargs:
                ...
                raise TypeError(
                    f"Cannot pass parameter(s) to an existing session: {sess_kwargs}"
                )
            else:
...
def request_list(
    ...
    if kwargs.pop("nohup", None):
        ...
        return [
            LazyTLSRequest(
                method, u, *args, **kwargs, executor=executor, raise_exception=False
            )
            for u in url
        ]
...
@overload
def request(
    method: str, url: str, *args, **kwargs
) -> Union[Response, LazyTLSRequest]: ...
...
def get(url: str, *args, **kwargs) -> Response:
    ...
    return retry_on_network_error(
        lambda: _get(url, *args, **add_redirects(kwargs, True))
    )
...
def options(url: str, *args, **kwargs) -> Response:
    ...
    fix_headers(kwargs)
    return retry_on_network_error(
        lambda: _options(url, *args, **add_redirects(kwargs, False))
    )
...
def head(url: str, *args, **kwargs) -> Response:
    ...
    fix_headers(kwargs)
    return retry_on_network_error(
        lambda: _head(url, *args, **add_redirects(kwargs, True))
    )
...
def post(url: str, *args, **kwargs) -> Response:
    ...
    fix_headers(kwargs)
    return retry_on_network_error(
        lambda: _post(url, *args, **add_redirects(kwargs, True))
    )
...
def put(url: str, *args, **kwargs) -> Response:
    ...
    fix_headers(kwargs)
    return retry_on_network_error(
        lambda: _put(url, *args, **add_redirects(kwargs, True))
    )
...
def patch(url: str, *args, **kwargs) -> Response:
    ...
    fix_headers(kwargs)
    return retry_on_network_error(
        lambda: _patch(url, *args, **add_redirects(kwargs, True))
    )
...
def delete(url: str, *args, **kwargs) -> Response:
    ...
    fix_headers(kwargs)
    return retry_on_network_error(
        lambda: _delete(url, *args, **add_redirects(kwargs, False))
    )
...
def map(
    ...
    for inc in range(0, len(requests), size):
        ...
        try:
            ...
            failed_resp: FailedResponse = FailedResponse(
                e
            )  # create a FailedResponse object
            resps = [failed_resp] * len(
                requests_range
            )  # add None for each failed request
            for req in requests_range:
