...
def extract_cookies_to_jar(
    ...
    for header_name, header_values in response_headers.items():
        http_message._headers.extend(
            (header_name, header_value) for header_value in header_values
        )
...
def list_to_cookiejar(browser_cookies: list) -> RequestsCookieJar:
    ...
    for br_cookie in browser_cookies:
        ...
        cookiejar.set_cookie(
            create_cookie(
                **{key: value for key, value in br_cookie.items() if key in cookie_keys}
            )
        )
    return cookiejar
