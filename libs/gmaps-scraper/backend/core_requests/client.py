...
class TLSClient:
    ...
    detect_encoding: bool = (
        True  # only disable if you are confident the encoding is utf-8
    )
    ...
    def unpack_proxy(proxies: Dict[str, str]) -> str:
        ...
        if key not in SUPPORTED_PROXIES:
            raise ProxyFormatException(
                f"Proxy must be of the following type: {SUPPORTED_PROXIES}"
            )
        return value
    ...
    def build_request(
        ...
        request_payload = {
            ...
            "headers": (
                dict(headers) if isinstance(headers, CaseInsensitiveDict) else headers
            ),
    ...
    def build_response(
        ...
        if not response_object["isHistory"]:
            return self.build_response_obj(
                url, headers, response_object["response"], proxy
            )
    ...
    def execute_request(
        ...
        **kwargs,
        ...
        request_payload, headers = self.build_request(
        ...
        return self.build_response(
            url, headers, response_object, request_payload["proxyUrl"]
        )
