        ...
        def wrapper(*args, **kwargs):
            ...
            while tries < retries:
                ...
                try:
                    ...
                    if instances != ANY:
                        errors_only_instances = (
                            list(
                                map(
                                    lambda el: el[0] if isinstance(el, tuple) else el,
                                    instances,
                                )
                            )
                            if instances
...
def retry_on_stale_element(
    _func: Optional[Callable] = None, *, retries=3, wait_time=1, raise_exception=True
):
...
def retry_on_request_failure(
    _func: Optional[Callable] = None, *, retries=5, wait_time=1, raise_exception=True
):
    def decorator(func):
        @retry_if_is_error(
            instances=ANY,
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                Exception
            ) as e:  # Catching a generic exception, can be replaced with specific exceptions
                print(f"Exception in {func.__name__}: {e}")  # Printing the exception
                result = (
                    on_exception_return_Value()
                    if callable(on_exception_return_Value)
                    else on_exception_return_Value
                )
                return result
