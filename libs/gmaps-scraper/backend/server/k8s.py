...
class K8s:
    ...
    def wait_for_workers(self):
        ...
        while ready_workers < total_workers:
            try:
                ...
                print(
                    f"Only {ready_workers} out of {total_workers} Workers are ready. Waiting for all Workers to be ready."
                )
                sleep(1)
            except Exception as e:
                print(f"An exception occurred: {str(e)}")
    ...
    def get_worker_count(self):
        depl = self.app_api.read_namespaced_stateful_set(
            name=self.pod_type + "-sts", namespace="default"
        )
    ...
    def get_ready_worker_count(self):
        ...
        pods = self.k8s_api.list_namespaced_pod(
            namespace="default", label_selector="app=" + self.pod_type
        )
    ...
    def get_worker_nodes(self):
        ...
        for pod in pods.items:
            node = {
                ...
                "current_capacity": {"request": 0, "browser": 0, "task": 0},
            }
            nodes.append(node)
        return nodes
