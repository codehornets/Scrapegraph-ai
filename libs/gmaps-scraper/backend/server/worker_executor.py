...
def make_request_with_retry(
    request_lambda, max_retries=6, initial_delay=1, backoff_factor=2, max_delay=60
):
    retries = -1
    delay = initial_delay
    ...
    while retries < max_retries:
        try:
            ...
            print(
                f"Attempt {retries + 1} failed with error: {e}. Retrying in {delay} seconds..."
            )

            time.sleep(delay)
            delay = min(delay * backoff_factor, max_delay) + random.uniform(
                0, 0.1 * delay
            )  # Adding jitter
...
class WorkerExecutor:
    ...
    def run_worker_task(self, task, node_name):
        Thread(
            target=self.perform_worker_task,
            args=(
                task,
                node_name,
            ),
        ...
        try:
            ...
            make_request_with_retry(
                lambda: requests.post(
                    "http://master-srv:8000/k8s/success",
                    json={
                        "task_id": task_id,
                        "task_type": scraper_type,
                        "data": task_data,
            ...
            make_request_with_retry(
                lambda: requests.post(
                    "http://master-srv:8000/k8s/fail",
                    json={
                        "task_id": task_id,
                        "task_type": scraper_type,
                        "task_result": exception_log,
                        "node_name": node_name,
                    },
                )
            )
