...
def istakinglong(iframe):
    return (
        ...
        in iframe.get_element_at_point(main_x, main_y, wait=None).text
    )
        ...
        if current_ray_id:
            ...
            if israychanged:
                ...
                while True:
                    ...
                    if iframe:
                        checkbox = iframe.get_element_at_point(
                            main_x, main_y, label_selector, wait=None
                        )
...
def solve_full_cf(driver):
    ...
    while not iframe:
        opponent = driver.get_bot_detected_by()
        if opponent != Opponent.CLOUDFLARE:
            return wait_till_document_is_ready(
                driver._tab, driver.config.wait_for_complete_page_load
            )
        sleep(1)
        iframe = get_iframe(driver)
...
def issuccess(iframe):
    return iframe.get_element_at_point(
        main_x,
        main_y,
        '#success[style="display: flex; visibility: visible;"]',
        wait=None,
    )
...
def isfailure(iframe):
    return iframe.get_element_at_point(
        main_x, main_y, '#fail[style="display: flex; visibility: visible;"]', wait=None
    )
