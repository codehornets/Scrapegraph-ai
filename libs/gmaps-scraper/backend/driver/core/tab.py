...
class Tab(Connection):
    ...
    def perform_get_element_at_point(self, x: int, y: int, raiseError=False):
        ...
        try:
            bid, __, nid = self.send(
                cdp.dom.get_node_for_location(
                    x=x, y=y, include_user_agent_shadow_dom=False
                )
            )
    ...
    def find_all(
        ...
        if timeout:
            while not results:
                results = self.find_elements_by_text(
                    text, type=type, exact_match=exact_match
                )
                if time.time() - now > timeout:
                    return []
                self.sleep(0.5)
    ...
    def select_all(
        ...
        results = (
            [
                item
                for item in results
                if item.node.node_name.lower() == node_name.lower()
            ]
            if node_name
            else results
        )
    ...
    def query_selector_all(
        ...
        try:
            node_ids = self.send(cdp.dom.query_selector_all(doc.node_id, selector))
...
if (resp instanceof Promise) {
        ...
        if remote_object:
            if return_by_value:
                if remote_object.value:
                    return json.loads(
                        util.get_remote_object_value(remote_object, core)
                    ).get("x")
    ...
    def get_content(self):
        ...
        """
        doc: cdp.dom.Node = self.send(cdp.dom.get_document(-1, True))
        return self.send(cdp.dom.get_outer_html(backend_node_id=doc.backend_node_id))
    ...
    def download_file(self, url: str, filename: Optional[PathLike] = None):
        ...
        filename = (
            filename if filename else datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        )
        code = r"""
         (elem) => {
    ...
    def set_download_path(self, path: PathLike):
        ...
        """
        self.send(
            cdp.browser.set_download_behavior(behavior="allow", download_path=path)
        )
