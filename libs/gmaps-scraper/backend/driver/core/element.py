...
def make_core_string(SCRIPT, args):
    expression = (
        r"const args = JSON.parse('ARGS'); const resp = (SCRIPT)(element);".replace(
            "SCRIPT", SCRIPT
        )
    )
    if args is not None:
...
class Element:
    ...
    def parent(self) -> typing.Union[Element, None]:
        ...
        """
        if not self.tree:
            raise ElementInitializationException(
    ...
    def mouse_move(self):
        ...
        x = center[0] - 4
        y = center[1] - 4
        self._tab.send(cdp.input_.dispatch_mouse_event("mouseMoved", x=x, y=y))
    ...
    return JSON.stringify({ "x": resp });
        ...
        try:
            ...
            if result and result[0]:
                if return_by_value:
                    return json.loads(
                        util.get_remote_object_value(result[0], core)
                    ).get("x")
    ...
    def download_video(
        self, filename, duration: typing.Optional[typing.Union[int, float]] = None
    ):
        """
        experimental option.
        ...
        self._tab.send(
            cdp.browser.set_download_behavior("allow", download_path=download_dir)
        )
    ...
    def is_video_downloaded(self):
        ...
        return (
            not isrecording
            and self.video_download_path
            and os.path.exists(self.video_download_path)
        )
