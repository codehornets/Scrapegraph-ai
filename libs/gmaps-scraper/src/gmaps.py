...
from backend.core.request_decorator import request
from backend.core.task_decorator import task
from backend.core.dontcache import DontCache

from . import scraper
from .sort_filter import filter_places, sort_dict_by_keys
...
def print_social_errors(credits_exhausted, not_subscribed, unknown_error):
    ...
    if credits_exhausted:
        print(
            f"Could not get social details for {len(credits_exhausted)} places due to credit exhaustion. Please consider upgrading your plan by visiting https://rapidapi.com/Chetan11dev/api/website-social-scraper-api/pricing to continue scraping social details."
        )

    if not_subscribed:
        print(
            f"Could not get social details for {len(not_subscribed)} places as you are not subscribed to Website Social Scraper. Please subscribe to a free plan by visiting https://rapidapi.com/Chetan11dev/api/website-social-scraper-api/pricing to scrape social details."
        )

    if unknown_error:
        print(
            f"Could not get social details for {len(unknown_error)} places due to Unknown Error."
        )
    ...
    for place in places:
        found_social_detail = next(
            (
                detail
                for detail in social_details
                if detail["place_id"] == place["place_id"]
            ),
            None,
        )
        ...
        if found_social_detail:
            ...
            if place.get("website") is not None:
                if should_scrape_socials:
                    place.update(
                        get_empty_data(
                            "Failed to get social details. Please check the logs for more information."
                        )
                    )
                else:
...
def process_result(
    ...
    if scrape_reviews:
        ...
        reviews_data = create_reviews_data(
            placed_with_reviews, reviews_max, reviews_sort, lang
        )
        reviews_details = scraper.scrape_reviews(
            reviews_data,
        )
        # print_social_errors
...
def google_maps_scraper(_, data):
    ...
    place_data = create_place_data(
        query, max_results, lang, geo_coordinates, zoom, links
    )
    places_obj = scraper.scrape_places(
        place_data,
    )
