...
def list_contains_string(string_list, target_string):
    target_string_lower = kebab_case(
        unicode_to_ascii(target_string)
    ).lower()  # Convert target string to lowercase
    for item in string_list:
        if (
            target_string_lower == kebab_case(unicode_to_ascii(item)).lower()
...
def filter_places(ls, filter_data):
    def fn(i):
        ...
        if min_rating is not None and (
            rating == "" or rating is None or rating < min_rating
        ):
            return False

        if max_rating is not None and (
            rating == "" or rating is None or rating > max_rating
        ):
            return False

        if min_reviews is not None and (
            reviews == "" or reviews is None or reviews < min_reviews
        ):
            return False

        if max_reviews is not None and (
            reviews == "" or reviews is None or reviews > max_reviews
        ):
            return False
